...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  // Direct link to the Drive image
  ...
  const theme = isHome 
    ? {
        navContainer: `border-white/10 ${scrolled ? 'bg-black/60 backdrop-blur-xl py-0' : 'bg-white/5 backdrop-blur-lg py-1'} text-white`,
        logoBg: 'bg-white/10',
    ...
    : {
        navContainer: `border-gray-200/60 ${scrolled ? 'bg-white/80 backdrop-blur-xl shadow-md py-0' : 'bg-white/60 backdrop-blur-lg shadow-sm py-1'} text-gray-900`,
        logoBg: 'bg-green-50',
    ...
    <div className="min-h-screen flex flex-col font-sans selection:bg-green-500 selection:text-white">
      <header className={`fixed top-0 left-0 right-0 z-50 flex justify-center transition-all duration-700 ease-expo ${scrolled ? 'mt-2' : 'mt-4 sm:mt-6'}`}>
        ...
        >
          <Link 
            to="/" 
            className="flex items-center group py-0.5 flex-shrink-0 transition-transform duration-300 hover:scale-[1.05]" 
          ...
          >
            <div className="relative flex items-center h-10 sm:h-12 lg:h-14">
               <img 
                 ...
                 className={`h-full w-auto object-contain transition-all duration-500 drop-shadow-md ${isHome && !scrolled ? 'brightness-110' : 'brightness-100'}`}
               />
            </div>
          </Link>
          ...
          <div className="hidden md:flex items-center pr-1 gap-3 flex-shrink-0">
             <div className={`
              ...
              `}>
                <Search className={`
                  absolute left-3.5 w-3.5 h-3.5 transition-colors duration-300 pointer-events-none
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-6">
            <div className="space-y-4">
              <div className="flex items-center gap-3 group">
                <div className="h-12 sm:h-14 overflow-hidden bg-white p-1 rounded-lg shadow-sm border border-gray-100">
                   <img src={LOGO_URL} alt="Ridge Realty Logo" className="h-full w-auto object-contain" />
                </div>
